...
// RedisStore v0.1.x takes a client object with scriptLoad/evalsha/decr/del methods.
// We wrap ioredis with error catching so Redis downtime fails open.
const redisClient = {
  ...
  evalsha: <TArgs extends unknown[], TData = unknown>(sha1: string, keys: string[], args: TArgs) =>
    (redis.evalsha(sha1, keys.length, ...keys, ...(args as string[])) as Promise<TData>).catch(() => null as TData),
  decr: (key: string) => redis.decr(key).catch(() => 0),
  del:  (key: string) => redis.del(key).catch(() => 0),
};
