...
- **UI:** the Products tab has an always-visible settings panel (enabled toggle, AI toggle + prompt) plus a collapsible "pending approval" panel (`GET /api/products?status=draft`) with Approve/Reject actions per card. Settings read/write via `GET/PATCH /api/products/auto-agent-settings`.
