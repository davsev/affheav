...
- **Feedback loop:** `products.status='rejected'` rows are kept (not deleted) specifically so two things can use them: dedup (never re-suggest the same URL) and, when AI is on, the relevance prompt is fed each subject's last 20 rejected titles so it learns to avoid similar picks over time — not just exact repeats. That covers human judgment; a second, real-traffic signal complements it — `getUnderperformingTitles()` finds `added_by='auto_agent'`/`status='active'` products that were sent at least 14 days ago and got ≤2 clicks since, and feeds those titles into the same relevance prompt as an equally-weighted "avoid" list, so products nobody actually clicks teach the AI too, not just ones a human explicitly rejected.
- **Dedup:** keyed primarily on `products.aliexpress_product_id` (the stable AliExpress product ID), not `long_url`. `long_url` stores the affiliate `promotion_link`, which AliExpress regenerates on every API call — the same physical product gets a different link each time, so a URL-only dedup check silently fails and the same product keeps resurfacing as "new" (this was a real bug; fixed by adding the `aliexpress_product_id` column and keying both cross-run and within-run dedup off it, matching the pattern `discoveryAgent.js`/`product_suggestions.aliexpress_id` already used correctly).
  - **Near-duplicate titles (different sellers, same item):** ID/URL dedup alone misses the same physical product listed by multiple suppliers, since each seller gets its own `product_id` — this is what caused e.g. the same LED strip to get drafted from five different suppliers in one run. `services/productDedup.js` (shared with `discoveryAgent.js`) compares significant-word Jaccard similarity between titles (`DUPLICATE_TITLE_THRESHOLD`, currently 0.5) and is applied twice: once filtering new candidates against every existing/known title, and once as a diversity pass across a single run's candidate pool (sorted by sales volume, greedily keeping the best of each near-duplicate cluster) before the AI relevance filter or insertion. Runs unconditionally, independent of whether AI mode is on.
  - **Durable memory (`agent_suggestion_history`):** an insert-only table, separate from `products`, recording every `(aliexpress_product_id, title)` this agent has ever drafted for a user. Unlike the `products`-table-based dedup above, it survives the user hard-deleting old (already-sent) products — without it, cleaning up sent products would erase the agent's memory and the same items would resurface as "new". Checked alongside `products` for both the ID set and the near-duplicate title list; written to (never read-modified) right after each draft insert.
